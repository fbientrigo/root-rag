...
class FairShipROOTExtractor:
    ...
    def _classify_header_provenance(self, header: str) -> str:
        ...
        if header.startswith("Fair") and not any(
            x in header for x in ["Ship", "ship", "Target", "target"]
        ):
            return "ROOT_ADJACENT"
        ...
        headers_list = [
            {
                "name": header,
                "count": len(files),
                "files": sorted(files)[:50],  # Limit to 50 files per header
            }
            for header, files in sorted(
                self.root_headers.items(), key=lambda x: len(x[1]), reverse=True
            )
        ]
        ...
        symbols_list = [
            {
                "name": symbol,
                "count": len(files),
                "files": sorted(files)[:50],  # Limit to 50 files per symbol
            }
            for symbol, files in sorted(
                self.root_symbols.items(), key=lambda x: len(x[1]), reverse=True
            )
        ]
            ...
            headers_by_prov[prov].append(
                {
                    "name": header,
                    "count": len(files),
                    "files": sorted(files)[:10],  # Limit to 10 files
                }
            )
    ...
    def generate_markdown_report(self, output_path: Path):
        ...
        sorted_headers = sorted(self.root_headers.items(), key=lambda x: len(x[1]), reverse=True)[
            :20
        ]
        ...
        sorted_symbols = sorted(self.root_symbols.items(), key=lambda x: len(x[1]), reverse=True)[
            :20
        ]
        ...
        lines.append("|----------|---------|---------|-------------|")
        lines.append(
            f"| ROOT_CORE | {header_prov_counts.get('ROOT_CORE', 0)} | {symbol_prov_counts.get('ROOT_CORE', 0)} | Canonical ROOT classes and APIs |"
        )
        lines.append(
            f"| ROOT_ADJACENT | {header_prov_counts.get('ROOT_ADJACENT', 0)} | {symbol_prov_counts.get('ROOT_ADJACENT', 0)} | FairRoot/VirtualMC/Geant4 framework |"
        )
        lines.append(
            f"| FAIRSHIP_LOCAL | {header_prov_counts.get('FAIRSHIP_LOCAL', 0)} | {symbol_prov_counts.get('FAIRSHIP_LOCAL', 0)} | FairShip detector/module code |"
        )
        lines.append(
            f"| UNCERTAIN | {header_prov_counts.get('UNCERTAIN', 0)} | {symbol_prov_counts.get('UNCERTAIN', 0)} | Ambiguous or insufficient evidence |"
        )
        lines.append("")
        ...
        root_core_headers = [
            (h, len(files))
            for h, files in self.root_headers.items()
        ...
        root_core_symbols = [
            (s, len(files))
            for s, files in self.root_symbols.items()
        ...
        for module, usage in sorted(
            self.module_usage.items(), key=lambda x: len(x[1]["files"]), reverse=True
        ):
            ...
            lines.append(
                f"| {module} | {files_count} | {headers_count} | {symbols_count} | {top_header} | {top_symbol} |"
            )
        ...
        lines.append(
            "- **Ignored directories**: .git, build, cmake-build*, dist, __pycache__, external"
        )
        lines.append("")
        ...
        lines.append(
            "3. **False negatives**: May miss ROOT usage in preprocessor macros or generated code"
        )
        lines.append(
            "4. **Symbol counts**: Counts file occurrences, not actual usage frequency within files"
        )
        lines.append("")
