...
STEPS = [
    ...
    Step(
        "PREFLIGHT",
        'test -f "$FAIRSHIP/macro/run_simScript.py"',
        "Confirm run_simScript.py exists.",
    ),
    Step(
        "PREFLIGHT", 'test -f "$FAIRSHIP/muonDIS/makeMuonDIS.py"', "Confirm makeMuonDIS.py exists."
    ),
    Step(
        "CODE_ANCHORED",
    ...
    Step(
        "PROVISIONAL",
        'python "$FAIRSHIP/macro/run_simScript.py" --MuDIS -f "<muonDis.root>" -i <firstEvent> -n <nEvents>',
        "Candidate MuDIS transport pattern.",
    ),
    Step("INSPECTION", "python <inspect_dis_branches.py>", "Check DIS tree and required branches."),
    Step(
        "INSPECTION",
        "python <inspect_cbmsim.py> <transport_output.root>",
        "Check cbmsim and CrossSection branch.",
    ),
    Step(
        "CLEANUP",
        "rm -f lxplus_muondis_preflight.log lxplus_muondis_inspection.log",
