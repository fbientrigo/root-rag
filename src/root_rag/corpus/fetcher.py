...
def resolve_git_ref(repo_url: str, root_ref: str) -> str:
    ...
    try:
        ...
        if not lines or not lines[0]:
            raise InvalidRefError(f"Reference '{root_ref}' not found in {repo_url}")
        ...
        raise GitOperationError(f"Git ls-remote timed out for {repo_url}") from err
    except InvalidRefError:
        raise
    except Exception as e:
        raise GitOperationError(f"Failed to resolve ref {root_ref}: {str(e)}") from e
...
def _clone_and_checkout(
    ...
    if result.returncode != 0:
        raise GitOperationError(f"Failed to clone {repo_url}: {result.stderr.strip()}")
    ...
    if result.returncode != 0:
        raise InvalidRefError(f"Cannot checkout ref '{root_ref}': {result.stderr.strip()}")
    ...
    if result.returncode != 0:
        raise GitOperationError(f"Failed to get commit SHA: {result.stderr.strip()}")
...
def fetch_corpus(
    ...
    with tempfile.TemporaryDirectory(prefix="root-rag-fetch-") as tmpdir:
        ...
        if actual_commit != resolved_commit:
            logger.warning(f"Resolved commit mismatch: {resolved_commit} vs {actual_commit}")
