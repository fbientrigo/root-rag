...
class RetrievalForestBackend(BaseRetrievalBackend):
    ...
    def search(self, query: str, top_k: int) -> list[EvidenceCandidate]:
        ...
        if self.fusion_method == "concat":
            ...
            fused.sort(
                key=lambda x: (
                    x.original_rank or 999,
                    self.profile_names.index(x.source_profile or ""),
                )
            )
        else:
    ...
    def _fuse(
        self, results_per_profile: list[list[EvidenceCandidate]], query: str
        ...
        if self.tie_breaker == "enhanced":
            ...
            ordered_anchors = sorted(
                ...
                key=lambda a: (
                    -fused_scores[a],
                    by_anchor[a].file_path,
                    by_anchor[a].start_line,
                    a,
                ),
            )
    ...
    def _deduplicate(self, candidates: list[EvidenceCandidate]) -> list[EvidenceCandidate]:
        ...
        for cand in candidates:
            ...
            for existing in unique_results:
                # Redundant when same file and line ranges overlap: max(starts) <= min(ends)
                if cand.file_path == existing.file_path and max(
                    cand.start_line, existing.start_line
                ) <= min(cand.end_line, existing.end_line):
                    is_redundant = True
                    break
            if not is_redundant:
