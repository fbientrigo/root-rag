...
def test_fairship_workflow_discovery_includes_target_files(tmp_path: Path):
    ...
    (tmp_path / "macro" / "run_simScript.py").write_text(
        "def run_simScript():\n    pass\n", encoding="utf-8"
    )
    (tmp_path / "python").mkdir()
    (tmp_path / "python" / "shipDigiReco.py").write_text(
        "def shipDigiReco():\n    pass\n", encoding="utf-8"
    )
    (tmp_path / "muonDIS").mkdir()
    (tmp_path / "muonDIS" / "makeMuonDIS.py").write_text(
        "def makeMuonDIS():\n    pass\n", encoding="utf-8"
    )
...
def test_chunk_corpus_with_fairship_workflow_profile_preserves_cpp_and_adds_scripts(tmp_path: Path):
    ...
    (tmp_path / "passive" / "ShipMuonShield.cxx").write_text(
        "class ShipMuonShield {};\n", encoding="utf-8"
    )
    (tmp_path / "macro").mkdir()
    (tmp_path / "macro" / "ShipReco.py").write_text(
        "def ShipReco():\n    return 0\n", encoding="utf-8"
    )
