...
""")
        ...
        assert "TT_scifi_plane_vert_volume" not in extractor.root_symbols, (
            "TT_scifi_plane should be rejected"
        )
        assert "TIMEDET_TIMEDET_H_" not in extractor.root_symbols, (
            "Include guard should be rejected"
        )
...
def test_provenance_classification():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        target_dir.mkdir()
        (target_dir / "TargetPoint.h").write_text(
            "#ifndef TARGETPOINT_H\nclass TargetPoint {};\n#endif"
        )
        (target_dir / "TargetTracker.h").write_text(
...
""")
        ...
        assert "TFile.h" in extractor.root_headers
        assert extractor.header_provenance.get("TFile.h") == "ROOT_CORE", (
            "TFile.h should be ROOT_CORE"
        )

        assert "TTree.h" in extractor.root_headers
        assert extractor.header_provenance.get("TTree.h") == "ROOT_CORE", (
            "TTree.h should be ROOT_CORE"
        )

        assert "TVector3.h" in extractor.root_headers
        assert extractor.header_provenance.get("TVector3.h") == "ROOT_CORE", (
            "TVector3.h should be ROOT_CORE"
        )

        assert "ROOT/RDataFrame.hxx" in extractor.root_headers
        assert extractor.header_provenance.get("ROOT/RDataFrame.hxx") == "ROOT_CORE", (
            "ROOT/RDataFrame.hxx should be ROOT_CORE"
        )
        ...
        if "TVirtualMC.h" in extractor.root_headers:
            assert extractor.header_provenance.get("TVirtualMC.h") == "ROOT_ADJACENT", (
                "TVirtualMC.h should be ROOT_ADJACENT"
            )

        if "TVirtualMC" in extractor.root_symbols:
            assert extractor.symbol_provenance.get("TVirtualMC") == "ROOT_ADJACENT", (
                "TVirtualMC should be ROOT_ADJACENT"
            )

        # Check FAIRSHIP_LOCAL classification
        assert "TargetPoint.h" in extractor.root_headers
        assert extractor.header_provenance.get("TargetPoint.h") == "FAIRSHIP_LOCAL", (
            "TargetPoint.h should be FAIRSHIP_LOCAL"
        )

        assert "TargetTracker.h" in extractor.root_headers
        assert extractor.header_provenance.get("TargetTracker.h") == "FAIRSHIP_LOCAL", (
            "TargetTracker.h should be FAIRSHIP_LOCAL"
        )

        assert "TVetoPoint.h" in extractor.root_headers
        assert extractor.header_provenance.get("TVetoPoint.h") == "FAIRSHIP_LOCAL", (
            "TVetoPoint.h should be FAIRSHIP_LOCAL"
        )

        assert "TTimeDet.h" in extractor.root_headers
        assert extractor.header_provenance.get("TTimeDet.h") == "FAIRSHIP_LOCAL", (
            "TTimeDet.h should be FAIRSHIP_LOCAL"
        )

        if "TargetPoint" in extractor.root_symbols:
            assert extractor.symbol_provenance.get("TargetPoint") == "FAIRSHIP_LOCAL", (
                "TargetPoint should be FAIRSHIP_LOCAL"
            )

        if "TargetTracker" in extractor.root_symbols:
            assert extractor.symbol_provenance.get("TargetTracker") == "FAIRSHIP_LOCAL", (
                "TargetTracker should be FAIRSHIP_LOCAL"
            )

        if "TVetoPoint" in extractor.root_symbols:
            assert extractor.symbol_provenance.get("TVetoPoint") == "FAIRSHIP_LOCAL", (
                "TVetoPoint should be FAIRSHIP_LOCAL"
            )

        if "TTimeDet" in extractor.root_symbols:
            assert extractor.symbol_provenance.get("TTimeDet") == "FAIRSHIP_LOCAL", (
                "TTimeDet should be FAIRSHIP_LOCAL"
            )
        ...
        assert (
            "ROOT_CORE" in prov_data["header_counts"] or "ROOT_CORE" in prov_data["symbol_counts"]
        )
...
""")
        ...
        assert extractor.symbol_provenance.get("TParticle") == "ROOT_CORE", (
            "TParticle should be ROOT_CORE"
        )
        ...
        assert extractor.symbol_provenance.get("TDatabasePDG") == "ROOT_CORE", (
            "TDatabasePDG should be ROOT_CORE (prefix fallback)"
        )

        assert "TParticlePDG" in extractor.root_symbols, "TParticlePDG should be detected"
        assert extractor.symbol_provenance.get("TParticlePDG") == "ROOT_CORE", (
            "TParticlePDG should be ROOT_CORE"
        )

        assert "TMCProcess" in extractor.root_symbols, "TMCProcess should be detected"
        assert extractor.symbol_provenance.get("TMCProcess") == "ROOT_CORE", (
            "TMCProcess should be ROOT_CORE"
        )
        ...
        assert extractor.symbol_provenance.get("TPythia6") == "ROOT_CORE", (
            "TPythia6 should be ROOT_CORE (prefix fallback)"
        )
        ...
        assert extractor.symbol_provenance.get("TPythia8") == "ROOT_CORE", (
            "TPythia8 should be ROOT_CORE (prefix fallback)"
        )

        assert "TPythia8Decayer" in extractor.root_symbols, "TPythia8Decayer should be detected"
        assert extractor.symbol_provenance.get("TPythia8Decayer") == "ROOT_CORE", (
            "TPythia8Decayer should be ROOT_CORE"
        )

        # Verify utility classes are ROOT_CORE
        assert "TNtuple" in extractor.root_symbols, "TNtuple should be detected"
        assert extractor.symbol_provenance.get("TNtuple") == "ROOT_CORE", (
            "TNtuple should be ROOT_CORE"
        )

        assert "TStopwatch" in extractor.root_symbols, "TStopwatch should be detected"
        assert extractor.symbol_provenance.get("TStopwatch") == "ROOT_CORE", (
            "TStopwatch should be ROOT_CORE"
        )

        assert "TMemFile" in extractor.root_symbols, "TMemFile should be detected"
        assert extractor.symbol_provenance.get("TMemFile") == "ROOT_CORE", (
            "TMemFile should be ROOT_CORE"
        )

        assert "TObjString" in extractor.root_symbols, "TObjString should be detected"
        assert extractor.symbol_provenance.get("TObjString") == "ROOT_CORE", (
            "TObjString should be ROOT_CORE"
        )

        assert "TMatrixD" in extractor.root_symbols, "TMatrixD should be detected"
        assert extractor.symbol_provenance.get("TMatrixD") == "ROOT_CORE", (
            "TMatrixD should be ROOT_CORE"
        )

        # Verify graphics classes are ROOT_CORE
        assert "TStyle" in extractor.root_symbols, "TStyle should be detected"
        assert extractor.symbol_provenance.get("TStyle") == "ROOT_CORE", (
            "TStyle should be ROOT_CORE"
        )

        assert "TLatex" in extractor.root_symbols, "TLatex should be detected"
        assert extractor.symbol_provenance.get("TLatex") == "ROOT_CORE", (
            "TLatex should be ROOT_CORE"
        )

        assert "TLegend" in extractor.root_symbols, "TLegend should be detected"
        assert extractor.symbol_provenance.get("TLegend") == "ROOT_CORE", (
            "TLegend should be ROOT_CORE"
        )
