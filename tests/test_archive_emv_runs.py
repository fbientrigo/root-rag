...
def _write_summary(root: Path, run_id: str, status: str, stamp: int) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        json.dumps({"run_id": run_id, "acceptance_gate_status": status}), encoding="utf-8"
    )
...
def test_dry_run_moves_nothing(tmp_path: Path) -> None:
    ...
    try:
        ...
        manifest = module.archive_runs(
            execute=False, keep_latest_pass=True, include_failed=True, older_than_days=None
        )
...
def test_execute_moves_only_allowed_files(tmp_path: Path) -> None:
    ...
    try:
        _prepare_fixture(tmp_path)
        manifest = module.archive_runs(
            execute=True, keep_latest_pass=True, include_failed=True, older_than_days=None
        )
...
def test_overlapping_run_ids_do_not_cross_claim_artifacts(tmp_path: Path) -> None:
    ...
    try:
        ...
        manifest = module.archive_runs(
            execute=False, keep_latest_pass=False, include_failed=True, older_than_days=None
        )
