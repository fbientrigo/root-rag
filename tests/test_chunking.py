...
def test_chunk_id_is_deterministic(cpp_repo_fixture):
    ...
    for c1, c2 in zip(chunks1, chunks2):
        assert c1.chunk_id == c2.chunk_id, f"Chunk IDs differ: {c1.chunk_id} vs {c2.chunk_id}"
...
def test_chunk_respects_version_metadata(cpp_repo_fixture):
    ...
    for chunk in chunks:
        assert chunk.root_ref == root_ref, f"Root ref mismatch: {chunk.root_ref} vs {root_ref}"
