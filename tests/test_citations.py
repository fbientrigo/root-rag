...
def test_chunk_content_is_nonempty_and_bounded(cpp_repo_fixture):
    ...
    for chunk in chunks:
        ...
        assert len(chunk.content) < 1_000_000, f"Content exceeds 1MB: {len(chunk.content)} bytes"
...
def test_doxygen_detection_sets_flag(cpp_repo_fixture):
    ...
    assert len(doxygen_chunks) > 0, "Should detect at least one chunk with Doxygen markers"
