...
def _write_decisions(
    ...
    *,
    ...
    reason: str,
    reviewer_notes: str,
    file_path: str = "muonDIS/makeMuonDIS.py",
) -> None:
    payload = {
        "review_id": "review-1",
...
def test_scaffold_defaults_to_needs_context() -> None:
    payload = yaml.safe_load(
        Path("benchmarks/muon_dis/qrels_review_decisions.yaml").read_text(encoding="utf-8")
    )
...
def test_dry_run_does_not_modify_qrels(tmp_path: Path) -> None:
    ...
    try:
        ...
        _write_decisions(
            decisions, decision="APPROVED", reason="valid anchor", reviewer_notes="reviewed"
        )
        _write_qrels(qrels)
...
def test_rejected_is_not_promoted(tmp_path: Path) -> None:
    ...
    try:
        ...
        _write_candidates(candidates)
        _write_decisions(
            decisions, decision="REJECTED", reason="not relevant", reviewer_notes="skip"
        )
        _write_qrels(qrels)
