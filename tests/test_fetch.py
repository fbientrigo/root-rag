...
from root_rag.corpus import InvalidRefError, Manifest, fetch_corpus
...
def test_fetch_writes_manifest_for_tag(tmp_path, git_repo_fixture):
    ...
    cache_dir / corpus_id / "manifest.json"
    # Note: actual corpus_id format might differ, but file should exist
    assert any(
        (cache_dir / d / "manifest.json").exists() for d in cache_dir.iterdir() if d.is_dir()
    )
